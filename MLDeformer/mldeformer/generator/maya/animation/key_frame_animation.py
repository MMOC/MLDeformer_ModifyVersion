...
class KeyFrameAnimation():
    ...
    # Modify
    @staticmethod
    def Copy_keyframes(start_time=0, end_time=-1):
        if end_time >= start_time:
            cmds.copyKey(time=(start_time, end_time))

    # Modify
    @staticmethod
    def Paste_keyframes(start_time=0, end_time=-1):
        if end_time >= start_time:
            cmds.pasteKey(time=(start_time, end_time), option='merge')
    ...
    def om_set_keyframes(ctrl_name, attr_name, key_times, key_values):
        ...
        #     mcurve.setValue(om.MTime(key_times[i].asUnits(om.MTime.uiUnit())), key_values[i])
