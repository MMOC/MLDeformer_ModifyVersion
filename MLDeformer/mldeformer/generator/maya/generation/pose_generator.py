...
def generate_samples_from_gui(event_handler, display_time=0.0):
    ...
    # Modify
    # Anim_End_Frame = deformer_config.OriginEnd_Frame
    Anim_End_Frame = deformer_config.num_samples / 10
    ...
    if deformer_config.Visibility == 1:
        if key_frame_anim.keyframes_exist(start_frame, Anim_End_Frame):
            ...
            # Modify
            for i in range(0, int(Anim_End_Frame)):

                cmds.currentTime(i)

                # Specify the object frame range
                num_copies = 10

                # Copy the transform of each bone and its sub-skeleton for each time interval
                for copy_index in range(num_copies):

                    # for bone_name in bone_names:
                    bone_name = ""
                    if target_controller_attributes is not None:
                        FirstBoneName, FirstBoneValue = target_controller_attributes[0].split('.')
                        bone_name = cmds.listRelatives(FirstBoneName, parent=True)

                    translate = cmds.xform(bone_name, q=True, worldSpace=False, relative=True, translation=True)
                    rotate = cmds.xform(bone_name, q=True, worldSpace=False, relative=True, rotation=True)

                    idx = 0
                    # Check rnd_attr_values
                    for ctrl_attr in target_controller_attributes:
                        ctrl, attr = ctrl_attr.split('.')
                        if bone_name == ctrl:
                            attr_values = key_frame_anim.attr_values[idx]
                            if 'rotate' in attr:
                                if 'X' in attr:
                                    rotate[0] += attr_values[i+(copy_index+1)]
                                elif 'Y' in attr:
                                    rotate[1] += attr_values[i+(copy_index+1)]
                                else:
                                    rotate[2] += attr_values[i+(copy_index+1)]
                            elif 'translate' in attr:
                                if 'X' in attr:
                                    translate[0] += attr_values[i+(copy_index+1)]
                                elif 'Y' in attr:
                                    translate[1] += attr_values[i+(copy_index+1)]
                                else:
                                    translate[2] += attr_values[i+(copy_index+1)]
                            idx += 1
                    ...
                    cmds.xform(bone_name, translation=translate, ws=False)
                    cmds.xform(bone_name, rotation=rotate, ws=False)

                    cmds.setKeyframe(bone_name, t=i+(copy_index+1)*(Anim_End_Frame-start_frame)+1)

                    sub_skeleton = cmds.listRelatives(bone_name, ad=True, type='joint')
                    if sub_skeleton:
                        for sub_bone_name in sub_skeleton:
                            btranslate = cmds.xform(sub_bone_name, q=True, worldSpace=False, relative=True, translation=True)
                            brotate = cmds.xform(sub_bone_name, q=True, worldSpace=False, relative=True, rotation=True)

                            # Check rnd_attr_values
                            for bctrl_attr in target_controller_attributes:
                                bctrl, battr = bctrl_attr.split('.')
                                if sub_bone_name == bctrl:
                                    battr_values = key_frame_anim.attr_values[idx]
                                    if 'rotate' in battr:
                                        if 'X' in battr:
                                            brotate[0] += battr_values[i+(copy_index+1)]
                                        elif 'Y' in battr:
                                            brotate[1] += battr_values[i+(copy_index+1)]
                                        else:
                                            brotate[2] += battr_values[i+(copy_index+1)]
                                    elif 'translate' in battr:
                                        if 'X' in battr:
                                            btranslate[0] += battr_values[i+(copy_index+1)]
                                        elif 'Y' in battr:
                                            btranslate[1] += battr_values[i+(copy_index+1)]
                                        else:
                                            btranslate[2] += battr_values[i+(copy_index+1)]
                                    idx += 1

                            cmds.xform(sub_bone_name, translation=btranslate, ws=False)
                            cmds.xform(sub_bone_name, rotation=brotate, ws=False)
                            cmds.setKeyframe(sub_bone_name, t=i+(copy_index+1)*(Anim_End_Frame-start_frame)+1)
